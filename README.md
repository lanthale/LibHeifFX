...
**JDK 20 and JavaFX 20 is required for v1.2.3** because of the foreign linker API usage and important changes (Windows) for threading happenend in Panama in JDK20
...
- The lib is using heavily the foreign memory API from Java 19 including functions from the project Panama and therefore the tool `JExtract` (Available only in JDK 19/20)
